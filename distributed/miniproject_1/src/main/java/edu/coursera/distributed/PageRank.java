...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Integer> p1 = sites.flatMapToPair(s -> {
            ArrayList<Tuple2<Integer, Integer>> a = new ArrayList<Tuple2<Integer, Integer>>();
            Iterator i = s._2().edgeIterator();
            while (i.hasNext()) {
                a.add(new Tuple2(s._1(), i.next()));
            }
            return a;
        });
        JavaPairRDD<Integer, Double> p2 = sites.join(ranks).mapValues(s ->
        {
            return s._2() / s._1().getNEdges();
        });
        JavaPairRDD<Integer, Double> p3 = p1.join(p2).flatMapToPair(s -> {
            ArrayList<Tuple2<Integer, Double>> a = new ArrayList<Tuple2<Integer, Double>>();
            a.add(s._2());
            return a;
        });
        return p3.reduceByKey((acc, s) -> {
            return acc + s;
        }).mapValues(s -> {
            return 0.15 + s * 0.85;
        });
